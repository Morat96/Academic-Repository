...
bool loadImages(string path, vector<Mat>& input_imgs) {
    ...
    for (size_t i=0; i<count; i++) {
        ...
        if( input.empty() )
        {
            cout <<  "Could not open or find image of the path: " << fn[i] << endl ;
            return false;
        }
        input_imgs.push_back(input);
    }
    return true;
}
...
int main()
{
    ...
    // since this phase may require some seconds, this output allows the user
    // to understand that the program is processing
    cout << "Looking for license plates positions in images" << endl;
    ...
    if (!inFile) {
        cerr << "Unable to open file licencePlates.txt";
